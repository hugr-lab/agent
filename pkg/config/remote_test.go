...
func TestLoadRemote_MergesAgentTypeAndOverride(t *testing.T) {
	...
	cfg, err := LoadRemote(context.Background(), q, "agt_1", boot)
	require.NoError(t, err)
...
func TestLoadRemote_MissingAgentErrors(t *testing.T) {
	...
	_, err := LoadRemote(context.Background(), q, "ghost", boot)
	require.Error(t, err)
...
func TestLoadRemote_AuthEnvExpansion(t *testing.T) {
	...
	cfg, err := LoadRemote(context.Background(), q, "agt_2", boot)
	require.NoError(t, err)
...
func TestLoadRemote_InputValidation(t *testing.T) {
	_, err := LoadRemote(context.Background(), nil, "x", &BootstrapConfig{})
	require.ErrorContains(t, err, "querier")

	boot := baseBoot(t)
	q := &stubQuerier{}
	_, err = LoadRemote(context.Background(), q, "", boot)
	require.ErrorContains(t, err, "agentID")

	_, err = LoadRemote(context.Background(), q, "x", nil)
