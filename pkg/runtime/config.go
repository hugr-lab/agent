// RuntimeConfig + loaders. RuntimeConfig composes YAML-shaped sub-
// configs owned by their domain packages (memory, chatcontext,
// skills, models, a2a, devui, agent, tools, store/local) — pkg/runtime
// does not own those types, only the composition. The cross-cutting
// types declared in this file (HugrConfig, AuthConfig, *Config helpers)
// are read by both BootstrapConfig (env-only) and RuntimeConfig
// (yaml/identity-driven), which is why they live alongside Build.
package runtime
...
// LoadBootstrap + LoadLocal and returns the full RuntimeConfig.
// New callers should use LoadBootstrap directly and then dispatch
// to LoadLocal or LoadRemote depending on boot.Remote().
func Load(yamlPath string) (*RuntimeConfig, error) {
	boot, err := LoadBootstrap(".env")
	if err != nil {
		return nil, err
	}
	return LoadLocal(yamlPath, boot, nil)
}

// LoadLocal returns the full RuntimeConfig derived from .env env-
// defaults (carried in boot) plus the YAML file at yamlPath. Passing
// "" for yamlPath skips YAML loading (tests).
//
// When src is non-nil, the resolved identity (from src.Agent(ctx))
// overrides whatever the YAML's `agent:` block set. This lets the
// hybrid local-with-hub Source push hub-side ID/short_id/name down
// into RuntimeConfig.Identity even when a config.yaml exists.
func LoadLocal(yamlPath string, boot *BootstrapConfig, src identity.Source) (*RuntimeConfig, error) {
	if boot == nil {
	...
	v.SetDefault("AGENT_CONSTITUTION", "constitution/agent.md")
	...
	if src != nil {
		agent, err := src.Agent(context.Background())
		if err != nil {
...
func applyYAML(cfg *RuntimeConfig, path string) error {
...
func decodeAndFinalize(v *viper.Viper, cfg *RuntimeConfig) error {
