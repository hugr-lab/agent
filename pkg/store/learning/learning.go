...
func (c *Client) CreateHypothesis(ctx context.Context, hyp Hypothesis) (string, error) {
	if hyp.Content == "" {
		return "", fmt.Errorf("hubdb: CreateHypothesis requires Content")
	}
	if hyp.ID == "" {
		hyp.ID = id.New(id.PrefixHypothesis, c.agentShort)
	}
	if hyp.AgentID == "" {
		hyp.AgentID = c.agentID
	}
	if hyp.Status == "" {
		hyp.Status = "proposed"
	}
...
func (c *Client) ListPendingHypotheses(ctx context.Context, priority string, limit int) ([]Hypothesis, error) {
	if limit <= 0 {
		limit = 10
	}
...
func (c *Client) hypothesisReplace(ctx context.Context, hypID string, mutate func(*Hypothesis)) error {
	current, err := c.getHypothesis(ctx, hypID)
	if err != nil {
		return err
	}
	if current == nil {
...
func (c *Client) getHypothesis(ctx context.Context, hypID string) (*Hypothesis, error) {
	type row struct {
		ID             string  `json:"id"`
		AgentID        string  `json:"agent_id"`
		Content        string  `json:"content"`
	...
	rows, err := qh.RunQuery[[]row](ctx, c.querier,
		...
		map[string]any{"agent": c.agentID, "id": hypID},
		"hub.db.agent.hypotheses",
	)
	if err != nil {
		if errors.Is(err, types.ErrWrongDataPath) || errors.Is(err, types.ErrNoData) {
...
func (c *Client) MarkHypothesisChecking(ctx context.Context, hypID string) error {
	now := time.Now().UTC()
	return c.hypothesisReplace(ctx, hypID, func(h *Hypothesis) {
		h.Status = "checking"
		h.CheckedAt = &now
	})
}
...
func (c *Client) ConfirmHypothesis(ctx context.Context, hypID string, evidence, factID string) error {
	now := time.Now().UTC()
	return c.hypothesisReplace(ctx, hypID, func(h *Hypothesis) {
		h.Status = "confirmed"
		h.Result = evidence
...
func (c *Client) RejectHypothesis(ctx context.Context, hypID string, evidence string) error {
	now := time.Now().UTC()
	return c.hypothesisReplace(ctx, hypID, func(h *Hypothesis) {
		h.Status = "rejected"
		h.Result = evidence
...
func (c *Client) DeferHypothesis(ctx context.Context, hypID string) error {
	return c.hypothesisReplace(ctx, hypID, func(h *Hypothesis) {
		h.Status = "proposed"
	})
}
...
func (c *Client) ExpireOldHypotheses(ctx context.Context, maxAge time.Duration) (int, error) {
	type result struct {
	...
	resp, err := c.querier.Query(ctx,
		...
		map[string]any{"agent": c.agentID, "cutoff": cutoff},
	)
	if err != nil {
...
func (c *Client) CreateReview(ctx context.Context, r Review) (string, error) {
	existing, err := c.GetReview(ctx, r.SessionID)
	if err != nil {
		return "", err
	}
	if existing != nil {
	...
	if r.ID == "" {
		r.ID = id.New(id.PrefixReview, c.agentShort)
	}
	if r.AgentID == "" {
		r.AgentID = c.agentID
	}
...
func (c *Client) GetReview(ctx context.Context, sessionID string) (*Review, error) {
	rows, err := c.listReviews(ctx, map[string]any{
		"agent_id":   map[string]any{"eq": c.agentID},
...
// limit.
func (c *Client) ListPendingReviews(ctx context.Context, limit int) ([]Review, error) {
	if limit <= 0 {
		limit = 10
	}
	return c.listReviews(ctx, map[string]any{
		"agent_id": map[string]any{"eq": c.agentID},
		"status":   map[string]any{"eq": "pending"},
	}, limit, "ASC")
}

func (c *Client) listReviews(ctx context.Context, filter map[string]any, limit int, direction string) ([]Review, error) {
	type row struct {
		ID              string  `json:"id"`
		AgentID         string  `json:"agent_id"`
...
func (c *Client) CompleteReview(ctx context.Context, reviewID string, result ReviewResult) error {
	return c.replaceReview(ctx, reviewID, func(r *Review) {
		r.Status = "completed"
...
func (c *Client) FailReview(ctx context.Context, reviewID string, errMsg string) error {
	return c.replaceReview(ctx, reviewID, func(r *Review) {
		r.Status = "failed"
		r.Error = errMsg
		now := time.Now().UTC()
...
func (c *Client) replaceReview(ctx context.Context, reviewID string, mutate func(*Review)) error {
